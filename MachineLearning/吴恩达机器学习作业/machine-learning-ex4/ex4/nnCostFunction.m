...
z_3 = a_2*Theta2';
a_3 = sigmoid(z_3);
J = sum(sum(-(y_true.*log(a_3)+(1-y_true).*log(1-a_3))))/m;
CostReg = (sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)))*lambda/(2*m);
J = J + CostReg;

for t = 1:m
    delta_3 = a_3(t,:) - y_true(t,:);
    tmp = delta_3*Theta2;
    delta_2 = tmp(2:end).*sigmoidGradient(z_2(t,:));
